  ...
  await new Promise<void>(resolve => {
    ...
    var interval = setInterval(() => {
      ...
      for (let i = 0; i < popups.length; i++) {
        if (window.getComputedStyle(popups[i]).display === 'block' && i > 2) {
          ...
          for (let i = 0; i < numChildren; i++) {
            const headingItemContainer = headingListContainer.children[
              i
            ] as HTMLElement
            const headingText =
              headingItemContainer.children[0]?.children[1]?.innerHTML
            if (!headingText) {
          ...
          if (i === numChildren) {
            ...
            new Error(
              'unable to set heading type - popup found, but heading value not'
            )
          }
        }
      }
      if (numTries >= 10) {
        clearInterval(interval)
        throw new Error(
          'unable to set heading type - 10 attempts, no success :('
        )
      }
    }, 50)
  })
...
export const indent = async (num_string: string) => {
  ...
  for (let i = 0; i < menubarElement.children.length; i++) {
    ...
    if (menu.innerHTML === 'Format') {
      clickEl(menu)
      formatMenu = document.getElementsByClassName(
    ...
    var interval = setInterval(() => {
      ...
      for (let i = 0; i < popups.length; i++) {
        ...
        if (
          ...
          (popups[i].children[5] as HTMLElement).innerText.startsWith(
            'Increase indent'
          )
        ) {
          ...
          if (
            num < 0 &&
            (!decreaseButton ||
              decreaseButton.innerText !== 'Decrease indent\nCtrl+[')
          ) {
            throw new Error(
              'unable to change indentation - decrease button not found!'
            )
          }
          while (num > 0) {
      ...
      if (numTries >= 10) {
        clearInterval(interval)
        throw new Error(
          'unable to change indentation - 10 attempts, no success :('
        )
      }
    }, 50)
  })
...
export const bulletList = (num_string: string) => {
  ...
  if (listType <= 0 || listType > 6) {
    console.log(
      `invalid bullet list format type ${listType}: expected value in from 1 to 6!`
    )
    listType = 1
  }
  if (listType === 1) {
    ...
    const bulletOptionsButton = document.getElementById(
      'bulletedListPresetMenuButton'
    )
    if (!bulletOptionsButton) {
      throw new Error(
        'unable to create bullet list: no bullet options button found!'
      )
    }
    ...
    if (!optionClasses) {
      throw new Error(
        'unable to create bullet list: internal Docs Hotkey error'
      )
    }
